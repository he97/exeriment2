...
def build_finetune_optimizer(config, model, logger):
    ...
    if opt_lower == 'sgd':
        optimizer = optim.SGD(parameters, momentum=config.TRAIN.OPTIMIZER.MOMENTUM, nesterov=True,
                              lr=config.TRAIN.FINETUNE.BASE_LR, weight_decay=config.TRAIN.FINETUNE.WEIGHT_DECAY)
    elif opt_lower == 'adamw':
        optimizer = optim.AdamW(parameters, eps=config.TRAIN.OPTIMIZER.EPS, betas=config.TRAIN.OPTIMIZER.BETAS,
                                lr=config.TRAIN.FINETUNE.BASE_LR, weight_decay=config.TRAIN.FINETUNE.WEIGHT_DECAY)
...
def build_optimizer_c(C1, C2, config, logger):
    logger.info('>>>>>>>>>> Build Optimizer for Pre-training Stage')
    skip = {}
    skip_keywords = {}

    parameters = get_pretrain_param_groups(C1, logger, skip, skip_keywords)
    parameters += get_pretrain_param_groups(C2, logger, skip, skip_keywords)
