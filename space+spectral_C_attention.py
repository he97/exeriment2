...
def train_one_epoch(config, G_spatial, G_spectral, Decoder_spatial, Decoder_spectral,
    ...
    for batch_idx, data in enumerate(zip(src_train_loader, tgt_train_loader)):
        ...
        output_spatial = G_spatial(spatial_all, mask=None, classifier_mode=config.CLASSIFIER.MODE)
        output_spectral = G_spectral(spectral_all, mask=None, classifier_mode=config.CLASSIFIER.MODE)
...
def eval_train(config, finetune_train_src_loader, finetune_train_tgt_loader, G_spatial, G_spectral, mix_model, C1, C2, epoch):
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(zip(finetune_train_src_loader, finetune_train_tgt_loader)):
            ...
            s_spatial_feature = G_spatial(s_spatial, mask=None, classifier_mode=config.CLASSIFIER.MODE)
            s_spectral_feature = G_spectral(s_spectral, mask=None, classifier_mode=config.CLASSIFIER.MODE)
            output_s = mix_model(s_spatial_feature, s_spectral_feature)
            t_spatial_feature = G_spatial(t_spatial, mask=None, classifier_mode=config.CLASSIFIER.MODE)
            t_spectral_feature = G_spectral(t_spectral, mask=None, classifier_mode=config.CLASSIFIER.MODE)
...
def eval_one_epoch(config, G_spatial, G_spectral, mix_model, C1, C2, test_loader, epoch):
    ...
    with torch.no_grad():
        for batch_idx, (spatial,spatial_mask,spectral,spectral_mask, _)  in enumerate(test_loader):
            ...
            spatial_feature = G_spatial(spatial,mask=None, classifier_mode=config.CLASSIFIER.MODE)
            spectral_feature = G_spectral(spectral,mask=None, classifier_mode=config.CLASSIFIER.MODE)
