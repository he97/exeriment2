...
class DTransformer_simMIM_finetune(nn.Module):
    def __init__(
            self,
            *,
            in_chans,
        ...
        self.pos_embedding = nn.Parameter(torch.randn(1, in_chans // group_size + 1, dim))
        self.patch_to_embedding = nn.Linear(patch_dim, dim)
        self.cls_token = nn.Parameter(torch.randn(1, 1, dim))
        self.dropout = nn.Dropout(emb_dropout)
    ...
    def forward(self, img):
        ...
        if exists(self.mlp_head):
            x = self.mlp_head(x)
        return x

    @torch.jit.ignore
    def freeze(self):
        return {'attn_layers.layers.0', 'attn_layers.layers.1', 'attn_layers.layers.2', 'attn_layers.layers.3','patch_embed','cls_token','pos_embedding'}
...
def build_Dtransformer_finetune(config):
    # 师哥代码的参数
    # 总维度数？num_patches=dim？
    dim = config.DATA.DIM
    # 一个patch中的元素个数
    patch_dim = 512  # 512
    CLASS_NUM = config.DATA.CLASS_NUM
    model = DTransformer_simMIM_finetune(
        in_chans=dim,
        # patch_size= 0,
        # sum of patches
        num_patches=dim,
