...
def check_parameter(old, new):
    for x in old.keys():
        assert x in new.keys(), 'key not in module'
        if not torch.equal(old[x],new[x]):
            logger.info(f'{x} not equal')
        else:
            logger.info(f'{x} equal')
...
def main(config):
    ...
    for epoch in range(config.TRAIN.EPOCHS):
        # copy status
        # G_status = copy.deepcopy(G.state_dict())
        # C1_status = copy.deepcopy(C1.state_dict())
        # C2_status = copy.deepcopy(C2.state_dict())
        # decoder_status = copy.deepcopy(Decoder.state_dict())
        # train
        ...
        # dd = map(check_parameter, [G_status,C1_status,C2_status,decoder_status],[G.state_dict(),C1.state_dict(),C2.state_dict(),Decoder.state_dict()] )
        # # pretrain
